...
class PlaceList(Resource):
    ...
    def post(self):
        ...
        place_data['owner_id'] = current_user  # inject owner from JWT
...
class PlaceResource(Resource):
    ...
    def get(self, place_id):
        """Retrieve details of a specific place (public)"""
    ...
    def put(self, place_id):
        """Update a place (authenticated, owner or admin)"""
        place = facade.get_place(place_id)
        if not place:
            return {'error': 'Place not found'}, 404
    ...
    def delete(self, place_id):
        """Delete a place (authenticated, owner or admin)"""
        place = facade.get_place(place_id)
        if not place:
            return {'error': 'Place not found'}, 404
...
class PlaceAmenities(Resource):
    ...
    def post(self, place_id):
        """Add amenities to a place (authenticated, owner or admin)"""
        ...
        for amenity_info in amenities_data:
            amenity = facade.get_amenity(amenity_info['id'])
            if not amenity:
                return {'error': 'Invalid input data'}, 400
